...
import {IVault, IERC20} from "@cellar-contracts/lib/balancer-v2-monorepo/pkg/interfaces/contracts/vault/IVault.sol";
...
contract AuraERC4626AdaptorTest is MainnetStarterTest, AdaptorHelperFunctions {
    ...
    function setUp() external {
        ...
        balancerStablePoolExtension = new BalancerStablePoolExtension(
            priceRouter,
            IVault(vault)
        );
        ...
        uint256 price = uint256(
            IChainlinkAggregator(address(mockWethUsd)).latestAnswer()
        );
        ...
        price = uint256(
            IChainlinkAggregator(address(mockRethUsd)).latestAnswer()
        );
        settings = PriceRouter.AssetSettings(
            CHAINLINK_DERIVATIVE,
            address(mockRethUsd)
        ...
        underlyings[0] = WETH;
        underlyings[1] = rETH;
        BalancerStablePoolExtension.ExtensionStorage
            memory extensionStor = BalancerStablePoolExtension
                .ExtensionStorage({
                    poolId: bytes32(0),
                    poolDecimals: 18,
        ...
        settings = PriceRouter.AssetSettings(
            ...
            address(balancerStablePoolExtension)
        );
        priceRouter.addAsset(
            rETH_wETH_BPT,
            settings,
            abi.encode(extensionStor),
            168726469843
        ); // obtained via local testing in terminal
    ...
    function testDeposit(uint256 assets) external {
        ...
        uint256 assetsInAuraPool = auraRETHWETHBPTVault.balanceOf(
            address(cellar)
        ); // as per Aura BaseRewardPool4626.sol - this is ASSUMING 1:1 for BPT (vault asset) : AuraBPT (shareToken)
        ...
        assertEq(
            assetsInAuraPool,
            assets,
            "Assets should have been deposited into assetsInAuraPool 1:1."
        );
        assertEq(
            cellar.totalAssets(),
    ...
    function testWithdraw(uint256 assets) external {
        ...
        uint256 redeemedAssets = cellar.redeem(
            maxRedeem,
            address(this),
            address(this)
        ); // state mutative call to redeem assets to address(this)
    ...
    function testInterestAccrual(uint256 assets) external {
        ...
        bool claimExtras = true;
        {
            ...
            adaptorCalls[0] = _createBytesDataGetRewardsFromAuraPoolERC4626(
                address(aura_rETH_wETH_BPT),
                claimExtras
            );
            data[0] = Cellar.AdaptorCall({
                adaptor: address(auraERC4626Adaptor),
